...
/**
 * Ищет релевантные фрагменты Конституции для последнего вопроса пользователя.
 * Возвращает null, если поиск недоступен (не настроен Qdrant) или произошла
 * ошибка — вызывающий код должен продолжать работу без RAG-контекста в этом случае,
 * чтобы временный сбой векторного поиска не ронял весь чат.
 */
...
exports.handler = async (event) => {
  ...
  // Берём последнее сообщение пользователя как поисковый запрос для RAG
